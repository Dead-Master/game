...
// возьми бота `codex_v1`,
// переделай стратегию на v2 добавив ему немного осторожности
// чтобы он не так активно лез картами вперёд

final class CodexV2BotStrategy implements BotStrategyInterface
{
    ...
    private function scoreAttack(array $attacker, array $defender): int
    {
        ...
        $canCounter = !$kill && $isMelee && $this->canUnitCounterAttack($defender);
        ...
        if ($canCounter) {
            $score -= min($defenderAtk, $attackerHp) * 10;
    ...
    private function tryBestMoveConservative(GameApiClient $api, int $gameId, array $state, string $side, array &$actions): bool
    {
        ...
        foreach ($this->getOwnUnits($state, $side) as $u) {
            ...
            for ($x = 0; $x <= 4; $x++) {
                for ($y = 0; $y <= 2; $y++) {
                    ...
                    // Консервативность: не лезем вперёд без смысла
                    if ($improve <= 0) {
                        continue;
                    }
                    ...
                    $score = 0;
                    $score += $improve * 12; // меньше, чем в v1
                    $score -= $risk * 14;    // сильный штраф за риск

                    // В ранней игре держим строй ближе к базе
                    if ($round <= 3) {
        ...
        // Порог: если ход слишком рискованный — не двигаемся
    ...
    private function tryBestDeployConservative(GameApiClient $api, int $gameId, array $state, string $side, array &$actions): bool
    {
        ...
        foreach ($cells as $cell) {
            ...
            foreach ($hand as $card) {
                ...
                $score = 0;
                $score -= $distEnemyBase * 8; // сильнее штраф за rush
                $score += match ($type) {
                    'infantry' => 22,
                    'berserker' => 19,
                    'archer' => 16,
